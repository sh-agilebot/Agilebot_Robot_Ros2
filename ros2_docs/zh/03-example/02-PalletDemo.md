...
graph TD
    ...
    C[gbt_agilegaze<br>视觉识别节点]:::node
...
| 模块名称 | 功能描述 |
|----------|----------|
| **gbt_agilegaze** | 对接AgileGaze视觉系统，提供目标物体的中心坐标(x,y)和偏转角(c) |
...
sequenceDiagram
    ...
    participant AgileGaze as gbt_agilegaze<br>视觉识别节点💚
    ...
    StackingNode->>Service: 🔹 send_script.srv()<br>发送控制指令
    activate Service
...
| 元素类型 | 颜色/标记 | 示例 |
|----------|-----------|------|
| ROS节点 | 💚 绿色背景 | robot_Stacking, gbt_agilegaze |
...
5. **机器人执行**  
主控节点经`send_script.srv()`服务下发控制指令，机器人控制器执行抓取/码垛动作并反馈执行状态。
...
```
> AgileGaze接口详见：gbt_stacking_interface/AgileGaze.msg

### RVIZ码垛可视化服务
- **服务类型**：`gbt_stacking_interface/msg/PalletVisualizer`
- **服务描述**：更新码垛可视化点坐标
```bash
# 请求
...
2. 下载并安装上海捷勃特机器人ROS-driver软件包，请参考[安装指南](../01-installation/01-Environment_Installation.md)
3. 配置`robot_config.yaml`中的机器人IP，配置文件位置：`gbt_driver/config/robot_config.yaml`
