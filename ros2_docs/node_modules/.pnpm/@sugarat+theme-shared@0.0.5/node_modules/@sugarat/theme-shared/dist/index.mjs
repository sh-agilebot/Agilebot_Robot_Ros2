...
function getVitePressPages(vpConfig) {
  ...
  const vitepressDynamicRoutes = new Map(
    _dynamicRoutes?.map((r) => [
      r.fullPath,
      slash(path2.join(srcDir, r.route))
    ]) || []
  );
  const vitepressRewrites = new Map(
    Object.entries(vpConfig?.rewrites.map || {}).map(([key, value]) => [
      slash(path2.join(srcDir, key)),
      slash(path2.join(srcDir, value))
    ]) || []
  );
  for (const page of pages) {
    ...
    let file = path2.resolve(vpConfig.srcDir, page);
    const fileOrig = vitepressDynamicRoutes.get(file) || file;
    file = vitepressRewrites.get(file) || file;
    const relativePath = slash(path2.relative(srcDir, file));
...
function normalize(path3) {
  return decodeURI(path3).replace(HASH_OR_QUERY_RE, "").replace(INDEX_OR_EXT_RE, "$1");
}
...
function isExternal(path3) {
  return /^(?:[a-z]+:|\/\/)/i.test(path3);
}
