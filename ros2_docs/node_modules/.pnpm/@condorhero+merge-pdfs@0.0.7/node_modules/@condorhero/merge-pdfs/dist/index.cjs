...
async function mergePdfsCli() {
  ...
  if (!output.endsWith(".pdf"))
    output += ".pdf";
  (0, import_node_fs.writeFileSync)(output, outBuffer);
  process.stdout.write((0, import_colorette.greenBright)(`
 Saved to ${(0, import_node_path.join)(process.cwd(), argv.o)}
`));
}
...
var __dirname2 = (0, import_node_path2.dirname)((0, import_node_url.fileURLToPath)(importMetaUrl));
async function mergePDFs(entry) {
  ...
  if (absolutePathArr.length < 2) {
    process.stdout.write((0, import_colorette2.yellowBright)("At least two PDF files.\n"));
    process.exit(1);
  }
  const pyodide = await (0, import_pyodide.loadPyodide)({ indexURL: (0, import_node_path2.resolve)(__dirname2, "pyodide") });
