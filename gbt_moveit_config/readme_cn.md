...
> 注意：
> 1.设置完成之后，请手动修改`config/joint_limits.yaml`中的`max_velocity`字段。将整数改成浮点数
> 2.设置完成之后，请手动修改`config/moveit_controllers.yaml`中的`controller`属性，增加命名空间。如果没有，请手动添加。
> 比如：
>
> ```yaml
